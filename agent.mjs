...
// Service probes removed 2026-09-13: host suspended us for free-tier over-consumption
// (USAGE_EXCEEDED), and our own healthz/402/demo probes every run were part of that traffic.
const service = 'not probed (host suspended, USAGE_EXCEEDED — see FIELD-STUDY §3.5)'
...
let prevUsdc = null, prevSol = null, prevSolNative = null, prevActionable = 0, prevTokuCents = null
try {
  const lines = readFileSync(new URL('./history.jsonl', import.meta.url), 'utf8').trim().split('\n')
  if (lines.length) { const p = JSON.parse(lines[lines.length - 1]); if (typeof p.baseUsdc === 'number') prevUsdc = p.baseUsdc; if (typeof p.solUsdc === 'number') prevSol = p.solUsdc; if (typeof p.solNative === 'number') prevSolNative = p.solNative; prevActionable = p.dealwork?.actionable || 0; if (typeof p.toku?.balanceCents === 'number') prevTokuCents = p.toku.balanceCents }
} catch {}
...
const notify = delta > 0 || newContract || tokuDelta > 0
...
const snapshot = { ts: now, baseUsdc: usdc, delta, service, openTask, dealwork, toku, superteam, newListings: fresh }
